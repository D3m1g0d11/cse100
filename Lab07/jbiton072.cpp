...
void search(list<int>Table[], int size, int key){
    ...
    for(iter = Table[j].begin(); iter != Table[j].end(); ++iter, ++i){
        if(*iter == key) {
...
int main(int argc, const char * argv[])
{
    ...
    while(repeat){
        cin >> input;
        if(input=='i'){ // you insert the key into the table
            cin>>key;
            insert(Table, size, key);
        }
        else if(input=='s'){ // search the key in the table
            cin>>key;
            search(Table, size, key);
        }
        else if (input=='o'){ // output the table
            output(Table, size);
        }
        else if (input=='d'){ //delete the key from table
            cin>>key;
